...
I2C_Status I2C_Interrupt_Master::write_n_then_read_m(uint8_t addr, const uint8_t *src, size_t src_len, uint8_t* dest, size_t dest_len, bool without_stop, bool nostop, uint32_t timeout_ms) {
    ...
    for (byte_ctr = 0; byte_ctr < ilen; ++byte_ctr) {
        ...
        i2c->hw->data_cmd =
                ...
                bool_to_bit(last && !without_stop) << I2C_IC_DATA_CMD_STOP_LSB |
                src[byte_ctr];
    ...
    i2c->restart_on_next = without_stop;
...
int I2C_Interrupt_Master::write_n(uint8_t addr, const uint8_t *src, size_t len, bool nostop) {
    invalid_params_if(I2C, addr >= 0x80); // 7-bit addresses
    invalid_params_if(I2C, i2c_reserved_addr(addr));
    // Synopsys hw accepts start/stop flags alongside data items in the same
    // FIFO word, so no 0 byte transfers.
